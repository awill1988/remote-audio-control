...
var buttonWithLabelWithLabel = module.exports = function(target) {
...
/** 
	...
	Touch buttonWithLabel with three modes of interaction ("toggle", "impulse", and "aftertouch").
	```html
	<canvas nx="buttonWithLabel"></canvas>
	```
	<canvas nx="buttonWithLabel" style="margin-left:25px"></canvas>
*/
	...
	/** 
		...
		```js 
		buttonWithLabel1.on('*', function(data) {
			// some code using data.press, data.x, and data.y
		});
		```
		Or, if NexusUI is outputting OSC (e.g. if nx.sendsTo("ajax")), val will be broken into OSC messages: 
		```html 
		/buttonWithLabel1/press 1
		/buttonWithLabel1/x 37
		/buttonWithLabel1/y 126
		```
		*/
	this.val = {
		press: 0
	...
	```js 
	buttonWithLabel1.mode = "aftertouch" 
	```
	*/
...
util.inherits(buttonWithLabel, widget);

buttonWithLabel.prototype.init = function() {
	this.center = {
		x: this.GUI.w/2,
		y: this.GUI.h/2
	}
...
buttonWithLabel.prototype.draw = function() {
	...
	with (this.context) {
		...
		if (this.image !== null) {
			...
			ctx.font = "12px Arial";
			ctx.fillText(this.label,this.center.x-10,this.center.y-10);
...
buttonWithLabel.prototype.click = function(e) {
...
buttonWithLabel.prototype.move = function () {
	// use to track movement on the buttonWithLabel
	if (this.mode=="aftertouch") {
		this.val["x"] = this.clickPos.x;
		this.val["y"] = this.clickPos.y;
...
buttonWithLabel.prototype.release = function() {
	this.val["press"] = 0;
...
/** @method setImage 
	Turns the buttonWithLabel into an image buttonWithLabel with custom image. Sets the default (unclicked) buttonWithLabel image.
	@param {string} [src] Image source */
buttonWithLabel.prototype.setImage = function(image) {
	this.image = new Image();
	this.image.onload = this.draw.bind(this)
	this.image.src = image;
}

buttonWithLabel.prototype.setHoverImage = function(image) {
...
/** @method setTouchImage 
	Sets the image that will show when the buttonWithLabel is clicked.
	@param {string} [src] Image source */
buttonWithLabel.prototype.setTouchImage = function(image) {
